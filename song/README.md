...
| `get_shelter_coverage(region)` | 무더위쉼터 접근성 — 최근접 거리, 관내 쉼터 수, 사각지대 여부 |
...
```json
{
  ...
  "reasons": [
    "고령인구 비율 52.3%로 높음",
    "가장 가까운 무더위쉼터가 1700m (도보 21분) — 도보권 밖"
  ]
}
```
...
- **쉼터 사각지대는 마을 중심에서 최근접 쉼터까지 도보 15분(1.2km) 초과**로 판정합니다.
  국립재난안전연구원의 400m(도보 5분) 기준은 *주민 집 → 쉼터* 거리인데, 저희가 가진
  좌표는 읍면동 중심점(몇 km 범위의 기하학적 중심)이라 그대로 적용하면 읍면동의 67%가
  사각지대가 되어 우선순위를 가릴 수 없습니다. 같은 보행 분속(80m)을 유지한 채
  측정 대상에 맞게 환산했습니다. `마을 중심 400m 내 쉼터 수`는 별도 지표로 함께 제공합니다.
- 사각지대는 읍면동 단위 판정입니다. 시군구는 수십 km에 걸쳐 있어 중심점 한 점으로는
  접근성을 말할 수 없으므로, 시군구의 쉼터 지표는 관할 읍면동 집계로 산출합니다.
